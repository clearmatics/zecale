...
void g1_well_formed_check_circuit(const libff::G1<npp> &g1_value)
{
    ASSERT_TRUE(g1_value.is_well_formed());

    libsnark::protoboard<Field> pb;
    libsnark::G1_variable<wpp> g1(pb, " g1");
    libsnark::G1_checker_gadget<wpp> check_g1(pb, g1, "check_g1");
    ...
    ASSERT_EQ(g1_value.is_well_formed(), pb.is_satisfied());
}
...
void g2_membership_check_circuit(const libff::G2<npp> &g2_value)
{
    ...
    ASSERT_EQ(expect_result, pb.is_satisfied());
}
...
TEST(BLS12_377_Membership_Check, G1NotWellFormedGadget)
{
    libff::Fq<npp> two = libff::Fq<npp>::one() + libff::Fq<npp>::one();
    const libff::G1<npp> g1_invalid(two, two, libff::Fq<npp>::one());
    ASSERT_FALSE(g1_invalid.is_well_formed());
    g1_well_formed_check_circuit(g1_invalid);
}
