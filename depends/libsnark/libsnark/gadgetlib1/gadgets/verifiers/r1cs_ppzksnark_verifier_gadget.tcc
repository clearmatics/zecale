...
{
    std::cout << "[r1cs_ppzksnark_verifier_gadget<ppT>::generate_r1cs_constraints] DEBUG1" << std::endl;
    PROFILE_CONSTRAINTS(this->pb, "precompute pvk")
    {
        compute_pvk->generate_r1cs_constraints();
    }

    std::cout << "[r1cs_ppzksnark_verifier_gadget<ppT>::generate_r1cs_constraints] DEBUG2" << std::endl;
    PROFILE_CONSTRAINTS(this->pb, "online verifier")
    {
        online_verifier->generate_r1cs_constraints();
    }
    std::cout << "[r1cs_ppzksnark_verifier_gadget<ppT>::generate_r1cs_constraints] DEBUG3" << std::endl;
}
