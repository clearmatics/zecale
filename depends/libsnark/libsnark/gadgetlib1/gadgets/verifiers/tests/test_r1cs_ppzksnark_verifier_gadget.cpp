...
void test_verifier(const std::string &annotation_A, const std::string &annotation_B)
{
    ...
    // Primary inputs of Zeth proofs are written over FieldT_A
    ...
    // Why is this var defined over ppT_B, and instantiated with `pi` later on, which is defined over ppT_A?
    // Well, actually this is normal.
    // We generate a Zeth proof over ppT_ZethCurve, and this proof is then verified in the aggregator circuit.
    // As such, the proof variable is defined over ppT_AggregatorCurve, since we are in the Aggregator circuit here.
