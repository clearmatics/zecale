...
contract AggregationVerifierBW6 {
    ...
    //
    // Note: the max value of `input` the unary representation of `batch_size`
    // e.g. the max value of `input` is (11111111)_2 if the `batch_size` is 8
    uint8 constant batch_size = 8;

    // All SNARK-based applications used with Zecale need to have the same
    // number of primary inputs. We use the "GGPR trick" here.
    ...
    function process_aggregate_tx(
        ...
        uint256 input,
        // Primary inputs of the nested proofs
