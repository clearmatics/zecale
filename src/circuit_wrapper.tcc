    ...
    TreeDepth>::generate_trusted_setup() const
{
    ...
    std::cout << "[circuit_wrapper - generate_trusted_setup] DEBUG1" << std::endl;

    joinsplit_gadget<FieldT, HashT, HashTreeT, NumInputs, NumOutputs, TreeDepth>
        g(pb);
    std::cout << "[circuit_wrapper - generate_trusted_setup] DEBUG2" << std::endl;
    g.generate_r1cs_constraints();

    std::cout << "[circuit_wrapper - generate_trusted_setup] DEBUG3" << std::endl;
    // Generate a verification and proving key (trusted setup)
    // and write them in a file
    std::cout << "[circuit_wrapper - generate_trusted_setup] DEBUG4" << std::endl;
        ...
        const provingKeyT<ppT> &proving_key) const
{
    ...
    // specified)
    // TODO: The function below only works for `libff::alt_bn128_G1` and so it isn commented out for now
    // to make the compilation go through for the MNT curves
