...
private:
    ...
    // message authentication tags (h_is), residual_bits (vin || vout))
    // Note: the root is a field element. We pack the nullifiers
    // (Inputs of JS = NumInputs) AND the commitments (Output of JS
    // = NumOutputs) AND the signature hash h_sig (+1) AND the message
    // authentication tags h_iS (+ NumInputs) AND the residual bits (vin || vout)
    std::array<
...
public:
    ...
    {
        std::cout << "[joinsplit.tcc] DEBUG1" << std::endl;
        // Block dedicated to generate the verifier inputs
        {
            ...
            // values in the residual bits
            packed_inputs[NumInputs + NumOutputs + 1 + NumInputs].allocate(
                pb,
                1, // (vin || vout) fits entirely in a field element
                FMT(this->annotation_prefix, " residual_bits"));

            // The primary inputs are:
            // [Root, NullifierS, CommitmentS, h_sig, h_iS, residual bits (vin || vout)].
            ...
            const size_t nb_inputs = 1 + nb_packed_inputs; // We add the root to the nb_inputs
            pb.set_input_sizes(nb_inputs);
            // ---------------------------------------------------------------
            std::cout << "[joinsplit.tcc] DEBUG2" << std::endl;
            ...
            phi.reset(new libsnark::digest_variable<FieldT>(
                ...
                FMT(this->annotation_prefix, " phi")
            ));
            h_sig.reset(new libsnark::digest_variable<FieldT>(
                pb,
                HashT::get_digest_len(),
                FMT(this->annotation_prefix, " h_sig")
            ));
            for (size_t i = 0; i < NumInputs; i++) {
            ...
            std::cout << "[joinsplit.tcc] DEBUG3" << std::endl;
            ...
            std::cout << "[joinsplit.tcc] DEBUG4" << std::endl;
            ...
            std::cout << "[joinsplit.tcc] DEBUG5" << std::endl;
            ...
            // Initialize the unpacked input corresponding to the value variables 
            // (vin, vout) of size smaller than (FieldT::capacity()) bits.
            // We obtain an unpacked value equal to v_in || v_out
            // We fill them here in reverse order because of
            // the use of .insert() function which adds a variable before a
            // given position
            std::cout << "[joinsplit.tcc] DEBUG6" << std::endl;
            {
                std::cout << "[joinsplit.tcc] DEBUG6.4" << std::endl;
                ...
                std::cout << "[joinsplit.tcc] DEBUG6.5" << std::endl;
            ...
            std::cout << "[joinsplit.tcc] DEBUG7" << std::endl;
            // [SANITY CHECK]
            ...
            std::cout << "[joinsplit.tcc] DEBUG7.1" << std::endl;
            ...
            std::cout << "[joinsplit.tcc] DEBUG7.2" << std::endl;
            ...
            std::cout << "[joinsplit.tcc] DEBUG7.3" << std::endl;
            ...
            std::cout << "[joinsplit.tcc] DEBUG7.4" << std::endl;
            // 3. Pack the h_sig
            ...
            std::cout << "[joinsplit.tcc] DEBUG7.5" << std::endl;
            // 4. Pack the h_iS
            for (size_t i = NumInputs + NumOutputs + 1;
                 i < NumInputs + NumOutputs + 1 + NumInputs;
                 i++) {
            ...
            std::cout << "[joinsplit.tcc] DEBUG7.6" << std::endl;
        ...
        std::cout << "[joinsplit.tcc] DEBUG8" << std::endl;
