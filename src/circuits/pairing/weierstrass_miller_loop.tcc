...
mnt_e_times_e_times_e_over_e_miller_loop_gadget<ppT>::mnt_e_times_e_times_e_over_e_miller_loop_gadget(libsnark::protoboard<FieldT> &pb,
...
result(result)
{
    const auto &loop_count = libsnark::pairing_selector<ppT>::pairing_loop_count;
    ...
    {
        fs[i].reset(new libsnark::Fqk_variable<ppT>(pb, FMT(annotation_prefix, " fs_%zu", i)));
    }
    ...
    for (long i = NAF.size()-1; i >= 0; --i)
    {
        ...
        doubling_steps4[dbl_id].reset(new mnt_miller_loop_dbl_line_eval<ppT>(pb,
                                                                             prec_P4, *prec_Q4.coeffs[prec_id],
                                                                             g_RR_at_P4s[dbl_id],
        ...
        ++f_id;
        dbl_muls4[dbl_id].reset(new Fqk_special_mul_gadget<ppT>(pb, (f_id + 1 == f_count ? result : *fs[f_id+1]), *g_RR_at_P4s[dbl_id], *fs[f_id], FMT(annotation_prefix, " dbl_muls4_%zu", dbl_id)));
        ++f_id;
        ++dbl_id;
        ...
        if (NAF[i] != 0)
        {
            ...
            addition_steps4[add_id].reset(new mnt_miller_loop_add_line_eval<ppT>(pb,
                                                                                 NAF[i] < 0,
                                                                                 prec_P4, *prec_Q4.coeffs[prec_id], *prec_Q4.Q,
                                                                                 g_RQ_at_P4s[add_id],
            ...
            ++f_id;
            add_muls4[add_id].reset(new Fqk_special_mul_gadget<ppT>(pb, (f_id + 1 == f_count ? result : *fs[f_id+1]), *g_RQ_at_P4s[add_id], *fs[f_id], FMT(annotation_prefix, " add_muls4_%zu", add_id)));
            ++f_id;
            ++add_id;
        }
    }
...
template<typename ppT>
void mnt_e_times_e_times_e_over_e_miller_loop_gadget<ppT>::generate_r1cs_constraints()
{
...
template<typename ppT>
void mnt_e_times_e_times_e_over_e_miller_loop_gadget<ppT>::generate_r1cs_witness()
{
    ...
    for (long i = NAF.size()-1; i >= 0; --i)
    {
        ...
        ++f_id;
        (f_id+1 == f_count ? result : *fs[f_id+1]).generate_r1cs_witness(fs[f_id]->get_element() * g_RR_at_P4s[dbl_id]->get_element().inverse());
        ...
        if (NAF[i] != 0)
        {
            ...
            ++f_id;
            (f_id+1 == f_count ? result : *fs[f_id+1]).generate_r1cs_witness(fs[f_id]->get_element() * g_RQ_at_P4s[add_id]->get_element().inverse());
...
void test_mnt_e_times_e_times_e_over_e_miller_loop(const std::string &annotation)
{
    ...
    libff::G1<other_curve<ppT> > P4_val = libff::Fr<other_curve<ppT> >::random_element() * libff::G1<other_curve<ppT> >::one();
    libff::G2<other_curve<ppT> > Q4_val = libff::Fr<other_curve<ppT> >::random_element() * libff::G2<other_curve<ppT> >::one();
    ...
    libsnark::precompute_G1_gadget<ppT> compute_prec_P1(pb, P1, prec_P1, "compute_prec_P1");
    libsnark::G1_precomputation<ppT> prec_P2;
    libsnark::precompute_G1_gadget<ppT> compute_prec_P2(pb, P2, prec_P2, "compute_prec_P2");
    libsnark::G1_precomputation<ppT> prec_P3;
    libsnark::precompute_G1_gadget<ppT> compute_prec_P3(pb, P3, prec_P3, "compute_prec_P3");
    libsnark::G1_precomputation<ppT> prec_P4;
    libsnark::precompute_G1_gadget<ppT> compute_prec_P3(pb, P4, prec_P4, "compute_prec_P4");
    libsnark::G2_precomputation<ppT> prec_Q1;
    libsnark::precompute_G2_gadget<ppT> compute_prec_Q1(pb, Q1, prec_Q1, "compute_prec_Q1");
    libsnark::G2_precomputation<ppT> prec_Q2;
    libsnark::precompute_G2_gadget<ppT> compute_prec_Q2(pb, Q2, prec_Q2, "compute_prec_Q2");
    libsnark::G2_precomputation<ppT> prec_Q3;
    libsnark::precompute_G2_gadget<ppT> compute_prec_Q3(pb, Q3, prec_Q3, "compute_prec_Q3");
    libsnark::G2_precomputation<ppT> prec_Q4;
    libsnark::precompute_G2_gadget<ppT> compute_prec_Q3(pb, Q4, prec_Q4, "compute_prec_Q4");
    ...
    libff::affine_ate_G1_precomp<other_curve<ppT> > native_prec_P4 = other_curve<ppT>::affine_ate_precompute_G1(P4_val);
    libff::affine_ate_G2_precomp<other_curve<ppT> > native_prec_Q4 = other_curve<ppT>::affine_ate_precompute_G2(Q4_val);
    libff::Fqk<other_curve<ppT> > native_result = (other_curve<ppT>::affine_ate_miller_loop(native_prec_P1, native_prec_Q1) *
                                            ...
                                            other_curve<ppT>::affine_ate_miller_loop(native_prec_P3, native_prec_Q3) *
                                            other_curve<ppT>::affine_ate_miller_loop(native_prec_P4, native_prec_Q4).inverse());

    assert(result.get_element() == native_result);
    printf("number of constraints for e times e times e over e Miller loop (Fr is %s)  = %zu\n", annotation.c_str(), pb.num_constraints());
}
