...
// Includes for the PGHR13 verifier
...
/// is succesfully verified by the groth16 verifier gadget
template<typename ppT_A, typename ppT_B>
void test_pghr13_verifier(const std::string &annotation_A, const std::string &annotation_B)
{
    typedef libff::Fr<ppT_A> FieldT_A;
    typedef libff::Fr<ppT_B> FieldT_B;
    ...
    const libsnark::r1cs_ppzksnark_keypair<ppT_A> keypair = libsnark::r1cs_ppzksnark_generator<ppT_A>(example.constraint_system);
    const libsnark::r1cs_ppzksnark_proof<ppT_A> pi = libsnark::r1cs_ppzksnark_prover<ppT_A>(keypair.pk, example.primary_input, example.auxiliary_input);
    bool bit = libsnark::r1cs_ppzksnark_verifier_strong_IC<ppT_A>(keypair.vk, example.primary_input, pi);
    ASSERT_TRUE(bit);
    ...
    libsnark::r1cs_ppzksnark_proof_variable<ppT_B> proof(pb, "proof");
    libsnark::r1cs_ppzksnark_verification_key_variable<ppT_B> vk(pb, vk_bits, primary_input_size, "vk");

    libsnark::pb_variable<FieldT_B> result;
    result.allocate(pb, "result");

    libsnark::r1cs_ppzksnark_verifier_gadget<ppT_B> verifier(pb, vk, primary_input_bits, elt_size, proof, result, "verifier");
...
{
    //test_pghr13_verifier<libff::mnt6_pp, libff::mnt4_pp>("mnt6", "mnt4");
    //test_pghr13_verifier<libff::mnt4_pp, libff::mnt6_pp>("mnt4", "mnt6");
